"""Provider-neutral contracts used by the independent Tie-Tu workflow."""
...
class SourceLedger:
    ...
    def add(self, record: SourceRecord) -> None:
        if record.source_id not in self.ids():
...
class ApprovalState:
    stages: Dict[str, str] = field(default_factory=lambda: {
        "topic": "pending", "brief": "pending", "card_plan": "pending",
        "pilot_image": "pending", "batch_generation": "pending",
        "preview": "pending", "publish": "pending",
    })
...
class ContentBrief:
    ...
    def from_dict(cls, payload: Dict[str, Any]) -> "ContentBrief":
        ...
        data["quality_gates"] = [QualityGate(**{
            "gate_id": item.get("gate_id", "brief"),
        ...
        data["source_ledger"] = SourceLedger(records=[SourceRecord(**item) for item in ledger.get("records", [])])
