...
NEGATIVES = (
    "watermark, logo, signature, extra words beyond the required exact Chinese copy, "
...
def build_portrait_spec(
    ...
    return {
        ...
        "direction": direction,
        "composition": f"{card.composition}; keep the required Chinese copy inside the clean text-safe area and away from the face",
        "text": card.overlay_text or (plan.title if card.role == "cover" else card.caption or card.purpose),
...
def render_portrait_prompt(spec: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f'Text (verbatim): "{spec.get("text", "")}". Render this exact Simplified Chinese copy in the same image-generation call, '
        "with clear hierarchy and mobile readability. Create one coherent photographed moment with no extra words, logos or watermarks."
    )
